...
properties grid, and analyst consensus + quarterly/yearly EPS forecast tables. A **Refresh Analyst
...
(Fundamentals). It adds no new tables.
...
- **FR-002** A detail endpoint returns, for a symbol: ticker row (Exchange, Active, IsFno,
  BarsAvailable), the latest `TickerTechnical` row, the latest `AnalystSnapshot` row, all current
...
- Adding new persisted columns or tables.
