CREATE TABLE [dbo].[USFundamentalIdeas]
(
    ...
    [IsStale]                           BIT           NOT NULL CONSTRAINT [DF_USFundamentalIdeas_IsStale] DEFAULT (0),
    [CapturedAt]                        DATETIME2(0)  NOT NULL CONSTRAINT [DF_USFundamentalIdeas_CapturedAt] DEFAULT (SYSUTCDATETIME()),
    [UpdatedAt]                         DATETIME2(0)  NOT NULL CONSTRAINT [DF_USFundamentalIdeas_UpdatedAt] DEFAULT (SYSUTCDATETIME()),

    CONSTRAINT [PK_USFundamentalIdeas] PRIMARY KEY CLUSTERED ([Ticker], [EarningsDate]),
    CONSTRAINT [FK_USFundamentalIdeas_USTickers] FOREIGN KEY ([Ticker]) REFERENCES [dbo].[USTickers]([Ticker]),
    INDEX [IX_USFundamentalIdeas_Active] NONCLUSTERED ([IsStale], [EarningsDate])
);
